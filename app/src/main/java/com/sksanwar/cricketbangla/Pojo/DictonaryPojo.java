package com.sksanwar.cricketbangla.Pojo;

import android.os.Parcel;
import android.os.Parcelable;

/**
 * Created by sksho on 18-Nov-17.
 */

public class DictonaryPojo implements Parcelable {

    @SuppressWarnings("unused")
    public static final Parcelable.Creator<DictonaryPojo> CREATOR = new Parcelable.Creator<DictonaryPojo>() {
        @Override
        public DictonaryPojo createFromParcel(Parcel in) {
            return new DictonaryPojo(in);
        }

        @Override
        public DictonaryPojo[] newArray(int size) {
            return new DictonaryPojo[size];
        }
    };


    private String home;
    private String commentary;
    private String scorecard;
    private String schedule;
    private String recent_results;
    private String setting;
    private String language;
    private String feedback;
    private String auto_refresh;
    private String target;
    private String ball_by_ball;
    private String over_by_over;
    private String squads;
    private String match_info;
    private String batsman;
    private String bowler;
    private String partnership;
    private String last_wkt;
    private String runs;
    private String recent_over;
    private String series_name;
    private String toss;
    private String venue;
    private String date;
    private String teams;
    private String umpires;
    private String umpire_1;
    private String umpire_2;
    private String umpire_3;
    private String referee;
    private String mom;
    private String mos;
    private String extras;
    private String extras_b;
    private String extras_wd;
    private String extras_nb;
    private String extras_lb;
    private String extras_p;
    private String yet_to_bat;
    private String header_crr;
    private String header_rr;
    private String header_run;
    private String header_ball;
    private String header_4s;
    private String header_6s;
    private String header_sr;
    private String header_over;
    private String header_overs;
    private String header_maiden;
    private String header_wickets;
    private String header_eco;
    private String fall_of_wicket;
    private String name;
    private String email;
    private String subject;
    private String message;
    private String powerplay;
    private String mandatory;
    private String batting;
    private String bowling;
    private String month_jan;
    private String month_feb;
    private String month_mar;
    private String month_apr;
    private String month_may;
    private String month_jun;
    private String month_jul;
    private String month_aug;
    private String month_sep;
    private String month_oct;
    private String month_nov;
    private String month_dec;
    private String week_sun;
    private String week_mon;
    private String week_tue;
    private String week_wed;
    private String week_thu;
    private String week_fri;
    private String week_sat;
    private String pull_to_refresh;
    private String loading;
    private String dob;
    private String birth_place;
    private String batting_style;
    private String bowling_style;
    private String ranking;
    private String career;
    private String rate_us;
    private String yes;
    private String no;
    private String no_internet;
    private String thank_you_contacting;
    private String career_best;
    private String enter_valid_email;
    private String fill_all_details;
    private String release_to_refresh;
    private String bench;
    private String career_stats;
    private String current_series;
    private String info;
    private String test;
    private String odi;
    private String t20;
    private String ipl;
    private String cl;
    private String debut;
    private String last_played;
    private String format;
    private String innings_list;
    private String overs;
    private String venue_time;
    private String teams_played_for;
    private String starts_in;
    private String follow_on;
    private String decl;
    private String points_table;
    private String country;
    private String points;
    private String rank;
    private String rating;
    private String matches;
    private String header_avg;
    private String top_teams;
    private String top_batsman;
    private String top_bowlers;
    private String top_allrounder;
    private String player;

    public DictonaryPojo() {
    }

    public DictonaryPojo(String home, String commentary, String scorecard, String schedule, String recent_results, String setting, String language,
                         String feedback, String auto_refresh, String target, String ball_by_ball, String over_by_over, String squads, String match_info,
                         String batsman, String bowler, String partnership, String last_wkt, String runs, String recent_over, String series_name,
                         String toss, String venue, String date, String teams, String umpires, String umpire_1, String umpire_2, String umpire_3,
                         String referee, String mom, String mos, String extras, String extras_b, String extras_wd, String extras_nb, String extras_lb,
                         String extras_p, String yet_to_bat, String header_crr, String header_rr, String header_run, String header_ball,
                         String header_4s, String header_6s, String header_sr, String header_over, String header_overs, String header_maiden,
                         String header_wickets, String header_eco, String fall_of_wicket, String name, String email, String subject, String message,
                         String powerplay, String mandatory, String batting, String bowling, String month_jan, String month_feb, String month_mar,
                         String month_apr, String month_may, String month_jun, String month_jul, String month_aug, String month_sep, String month_oct,
                         String month_nov, String month_dec, String week_sun, String week_mon, String week_tue, String week_wed, String week_thu,
                         String week_fri, String week_sat, String pull_to_refresh, String loading, String dob, String birth_place, String batting_style,
                         String bowling_style, String ranking, String career, String rate_us, String yes, String no, String no_internet,
                         String thank_you_contacting, String career_best, String enter_valid_email, String fill_all_details, String release_to_refresh,
                         String bench, String career_stats, String current_series, String info, String test, String odi, String t20, String ipl, String cl,
                         String debut, String last_played, String format, String innings_list, String overs, String venue_time, String teams_played_for,
                         String starts_in, String follow_on, String decl, String points_table, String country, String points, String rank, String rating, String matches,
                         String header_avg, String top_teams, String top_batsman, String top_bowlers, String top_allrounder, String player)

    {
        this.home = home;
        this.commentary = commentary;
        this.scorecard = scorecard;
        this.schedule = schedule;
        this.recent_results = recent_results;
        this.setting = setting;
        this.language = language;
        this.feedback = feedback;
        this.auto_refresh = auto_refresh;
        this.target = target;
        this.ball_by_ball = ball_by_ball;
        this.over_by_over = over_by_over;
        this.squads = squads;
        this.match_info = match_info;
        this.batsman = batsman;
        this.bowler = bowler;
        this.partnership = partnership;
        this.last_wkt = last_wkt;
        this.runs = runs;
        this.recent_over = recent_over;
        this.series_name = series_name;
        this.toss = toss;
        this.venue = venue;
        this.date = date;
        this.teams = teams;
        this.umpires = umpires;
        this.umpire_1 = umpire_1;
        this.umpire_2 = umpire_2;
        this.umpire_3 = umpire_3;
        this.referee = referee;
        this.mom = mom;
        this.mos = mos;
        this.extras = extras;
        this.extras_b = extras_b;
        this.extras_wd = extras_wd;
        this.extras_nb = extras_nb;
        this.extras_lb = extras_lb;
        this.extras_p = extras_p;
        this.yet_to_bat = yet_to_bat;
        this.header_crr = header_crr;
        this.header_rr = header_rr;
        this.header_run = header_run;
        this.header_ball = header_ball;
        this.header_4s = header_4s;
        this.header_6s = header_6s;
        this.header_sr = header_sr;
        this.header_over = header_over;
        this.header_overs = header_overs;
        this.header_maiden = header_maiden;
        this.header_wickets = header_wickets;
        this.header_eco = header_eco;
        this.fall_of_wicket = fall_of_wicket;
        this.name = name;
        this.email = email;
        this.subject = subject;
        this.message = message;
        this.powerplay = powerplay;
        this.mandatory = mandatory;
        this.batting = batting;
        this.bowling = bowling;
        this.month_jan = month_jan;
        this.month_feb = month_feb;
        this.month_mar = month_mar;
        this.month_apr = month_apr;
        this.month_may = month_may;
        this.month_jun = month_jun;
        this.month_jul = month_jul;
        this.month_aug = month_aug;
        this.month_sep = month_sep;
        this.month_oct = month_oct;
        this.month_nov = month_nov;
        this.month_dec = month_dec;
        this.week_sun = week_sun;
        this.week_mon = week_mon;
        this.week_tue = week_tue;
        this.week_wed = week_wed;
        this.week_thu = week_thu;
        this.week_fri = week_fri;
        this.week_sat = week_sat;
        this.pull_to_refresh = pull_to_refresh;
        this.loading = loading;
        this.dob = dob;
        this.birth_place = birth_place;
        this.batting_style = batting_style;
        this.bowling_style = bowling_style;
        this.ranking = ranking;
        this.career = career;
        this.rate_us = rate_us;
        this.yes = yes;
        this.no = no;
        this.no_internet = no_internet;
        this.thank_you_contacting = thank_you_contacting;
        this.career_best = career_best;
        this.enter_valid_email = enter_valid_email;
        this.fill_all_details = fill_all_details;
        this.release_to_refresh = release_to_refresh;
        this.bench = bench;
        this.career_stats = career_stats;
        this.current_series = current_series;
        this.info = info;
        this.test = test;
        this.odi = odi;
        this.t20 = t20;
        this.ipl = ipl;
        this.cl = cl;
        this.debut = debut;
        this.last_played = last_played;
        this.format = format;
        this.innings_list = innings_list;
        this.overs = overs;
        this.venue_time = venue_time;
        this.teams_played_for = teams_played_for;
        this.starts_in = starts_in;
        this.follow_on = follow_on;
        this.decl = decl;
        this.points_table = points_table;
        this.country = country;
        this.points = points;
        this.rank = rank;
        this.rating = rating;
        this.matches = matches;
        this.header_avg = header_avg;
        this.top_teams = top_teams;
        this.top_batsman = top_batsman;
        this.top_bowlers = top_bowlers;
        this.top_allrounder = top_allrounder;
        this.player = player;
    }

    protected DictonaryPojo(Parcel in) {
        home = in.readString();
        commentary = in.readString();
        scorecard = in.readString();
        schedule = in.readString();
        recent_results = in.readString();
        setting = in.readString();
        language = in.readString();
        feedback = in.readString();
        auto_refresh = in.readString();
        target = in.readString();
        ball_by_ball = in.readString();
        over_by_over = in.readString();
        squads = in.readString();
        match_info = in.readString();
        batsman = in.readString();
        bowler = in.readString();
        partnership = in.readString();
        last_wkt = in.readString();
        runs = in.readString();
        recent_over = in.readString();
        series_name = in.readString();
        toss = in.readString();
        venue = in.readString();
        date = in.readString();
        teams = in.readString();
        umpires = in.readString();
        umpire_1 = in.readString();
        umpire_2 = in.readString();
        umpire_3 = in.readString();
        referee = in.readString();
        mom = in.readString();
        mos = in.readString();
        extras = in.readString();
        extras_b = in.readString();
        extras_wd = in.readString();
        extras_nb = in.readString();
        extras_lb = in.readString();
        extras_p = in.readString();
        yet_to_bat = in.readString();
        header_crr = in.readString();
        header_rr = in.readString();
        header_run = in.readString();
        header_ball = in.readString();
        header_4s = in.readString();
        header_6s = in.readString();
        header_sr = in.readString();
        header_over = in.readString();
        header_overs = in.readString();
        header_maiden = in.readString();
        header_wickets = in.readString();
        header_eco = in.readString();
        fall_of_wicket = in.readString();
        name = in.readString();
        email = in.readString();
        subject = in.readString();
        message = in.readString();
        powerplay = in.readString();
        mandatory = in.readString();
        batting = in.readString();
        bowling = in.readString();
        month_jan = in.readString();
        month_feb = in.readString();
        month_mar = in.readString();
        month_apr = in.readString();
        month_may = in.readString();
        month_jun = in.readString();
        month_jul = in.readString();
        month_aug = in.readString();
        month_sep = in.readString();
        month_oct = in.readString();
        month_nov = in.readString();
        month_dec = in.readString();
        week_sun = in.readString();
        week_mon = in.readString();
        week_tue = in.readString();
        week_wed = in.readString();
        week_thu = in.readString();
        week_fri = in.readString();
        week_sat = in.readString();
        pull_to_refresh = in.readString();
        loading = in.readString();
        dob = in.readString();
        birth_place = in.readString();
        batting_style = in.readString();
        bowling_style = in.readString();
        ranking = in.readString();
        career = in.readString();
        rate_us = in.readString();
        yes = in.readString();
        no = in.readString();
        no_internet = in.readString();
        thank_you_contacting = in.readString();
        career_best = in.readString();
        enter_valid_email = in.readString();
        fill_all_details = in.readString();
        release_to_refresh = in.readString();
        bench = in.readString();
        career_stats = in.readString();
        current_series = in.readString();
        info = in.readString();
        test = in.readString();
        odi = in.readString();
        t20 = in.readString();
        ipl = in.readString();
        cl = in.readString();
        debut = in.readString();
        last_played = in.readString();
        format = in.readString();
        innings_list = in.readString();
        overs = in.readString();
        venue_time = in.readString();
        teams_played_for = in.readString();
        starts_in = in.readString();
        follow_on = in.readString();
        decl = in.readString();
        points_table = in.readString();
        country = in.readString();
        points = in.readString();
        rank = in.readString();
        rating = in.readString();
        matches = in.readString();
        header_avg = in.readString();
        top_teams = in.readString();
        top_batsman = in.readString();
        top_bowlers = in.readString();
        top_allrounder = in.readString();
        player = in.readString();
    }

    public String getHome() {
        return home;
    }

    public String getCommentary() {
        return commentary;
    }

    public String getScorecard() {
        return scorecard;
    }

    public String getSchedule() {
        return schedule;
    }

    public String getRecent_results() {
        return recent_results;
    }

    public String getSetting() {
        return setting;
    }

    public String getLanguage() {
        return language;
    }

    public String getFeedback() {
        return feedback;
    }

    public String getAuto_refresh() {
        return auto_refresh;
    }

    public String getTarget() {
        return target;
    }

    public String getBall_by_ball() {
        return ball_by_ball;
    }

    public String getOver_by_over() {
        return over_by_over;
    }

    public String getSquads() {
        return squads;
    }

    public String getMatch_info() {
        return match_info;
    }

    public String getBatsman() {
        return batsman;
    }

    public String getBowler() {
        return bowler;
    }

    public String getPartnership() {
        return partnership;
    }

    public String getLast_wkt() {
        return last_wkt;
    }

    public String getRuns() {
        return runs;
    }

    public String getRecent_over() {
        return recent_over;
    }

    public String getSeries_name() {
        return series_name;
    }

    public String getToss() {
        return toss;
    }

    public String getVenue() {
        return venue;
    }

    public String getDate() {
        return date;
    }

    public String getTeams() {
        return teams;
    }

    public String getUmpires() {
        return umpires;
    }

    public String getUmpire_1() {
        return umpire_1;
    }

    public String getUmpire_2() {
        return umpire_2;
    }

    public String getUmpire_3() {
        return umpire_3;
    }

    public String getReferee() {
        return referee;
    }

    public String getMom() {
        return mom;
    }

    public String getMos() {
        return mos;
    }

    public String getExtras() {
        return extras;
    }

    public String getExtras_b() {
        return extras_b;
    }

    public String getExtras_wd() {
        return extras_wd;
    }

    public String getExtras_nb() {
        return extras_nb;
    }

    public String getExtras_lb() {
        return extras_lb;
    }

    public String getExtras_p() {
        return extras_p;
    }

    public String getYet_to_bat() {
        return yet_to_bat;
    }

    public String getHeader_crr() {
        return header_crr;
    }

    public String getHeader_rr() {
        return header_rr;
    }

    public String getHeader_run() {
        return header_run;
    }

    public String getHeader_ball() {
        return header_ball;
    }

    public String getHeader_4s() {
        return header_4s;
    }

    public String getHeader_6s() {
        return header_6s;
    }

    public String getHeader_sr() {
        return header_sr;
    }

    public String getHeader_over() {
        return header_over;
    }

    public String getHeader_overs() {
        return header_overs;
    }

    public String getHeader_maiden() {
        return header_maiden;
    }

    public String getHeader_wickets() {
        return header_wickets;
    }

    public String getHeader_eco() {
        return header_eco;
    }

    public String getFall_of_wicket() {
        return fall_of_wicket;
    }

    public String getName() {
        return name;
    }

    public String getEmail() {
        return email;
    }

    public String getSubject() {
        return subject;
    }

    public String getMessage() {
        return message;
    }

    public String getPowerplay() {
        return powerplay;
    }

    public String getMandatory() {
        return mandatory;
    }

    public String getBatting() {
        return batting;
    }

    public String getBowling() {
        return bowling;
    }

    public String getMonth_jan() {
        return month_jan;
    }

    public String getMonth_feb() {
        return month_feb;
    }

    public String getMonth_mar() {
        return month_mar;
    }

    public String getMonth_apr() {
        return month_apr;
    }

    public String getMonth_may() {
        return month_may;
    }

    public String getMonth_jun() {
        return month_jun;
    }

    public String getMonth_jul() {
        return month_jul;
    }

    public String getMonth_aug() {
        return month_aug;
    }

    public String getMonth_sep() {
        return month_sep;
    }

    public String getMonth_oct() {
        return month_oct;
    }

    public String getMonth_nov() {
        return month_nov;
    }

    public String getMonth_dec() {
        return month_dec;
    }

    public String getWeek_sun() {
        return week_sun;
    }

    public String getWeek_mon() {
        return week_mon;
    }

    public String getWeek_tue() {
        return week_tue;
    }

    public String getWeek_wed() {
        return week_wed;
    }

    public String getWeek_thu() {
        return week_thu;
    }

    public String getWeek_fri() {
        return week_fri;
    }

    public String getWeek_sat() {
        return week_sat;
    }

    public String getPull_to_refresh() {
        return pull_to_refresh;
    }

    public String getLoading() {
        return loading;
    }

    public String getDob() {
        return dob;
    }

    public String getBirth_place() {
        return birth_place;
    }

    public String getBatting_style() {
        return batting_style;
    }

    public String getBowling_style() {
        return bowling_style;
    }

    public String getRanking() {
        return ranking;
    }

    public String getCareer() {
        return career;
    }

    public String getRate_us() {
        return rate_us;
    }

    public String getYes() {
        return yes;
    }

    public String getNo() {
        return no;
    }

    public String getNo_internet() {
        return no_internet;
    }

    public String getThank_you_contacting() {
        return thank_you_contacting;
    }

    public String getCareer_best() {
        return career_best;
    }

    public String getEnter_valid_email() {
        return enter_valid_email;
    }

    public String getFill_all_details() {
        return fill_all_details;
    }

    public String getRelease_to_refresh() {
        return release_to_refresh;
    }

    public String getBench() {
        return bench;
    }

    public String getCareer_stats() {
        return career_stats;
    }

    public String getCurrent_series() {
        return current_series;
    }

    public String getInfo() {
        return info;
    }

    public String getTest() {
        return test;
    }

    public String getOdi() {
        return odi;
    }

    public String getT20() {
        return t20;
    }

    public String getIpl() {
        return ipl;
    }

    public String getCl() {
        return cl;
    }

    public String getDebut() {
        return debut;
    }

    public String getLast_played() {
        return last_played;
    }

    public String getFormat() {
        return format;
    }

    public String getInnings_list() {
        return innings_list;
    }

    public String getOvers() {
        return overs;
    }

    public String getVenue_time() {
        return venue_time;
    }

    public String getTeams_played_for() {
        return teams_played_for;
    }

    public String getStarts_in() {
        return starts_in;
    }

    public String getFollow_on() {
        return follow_on;
    }

    public String getDecl() {
        return decl;
    }

    public String getPoints_table() {
        return points_table;
    }

    public String getCountry() {
        return country;
    }

    public String getPoints() {
        return points;
    }

    public String getRank() {
        return rank;
    }

    public String getRating() {
        return rating;
    }

    public String getMatches() {
        return matches;
    }

    public String getHeader_avg() {
        return header_avg;
    }

    public String getTop_teams() {
        return top_teams;
    }

    public String getTop_batsman() {
        return top_batsman;
    }

    public String getTop_bowlers() {
        return top_bowlers;
    }

    public String getTop_allrounder() {
        return top_allrounder;
    }

    public String getPlayer() {
        return player;
    }

    @Override
    public int describeContents() {
        return 0;
    }

    @Override
    public void writeToParcel(Parcel dest, int flags) {
        dest.writeString(home);
        dest.writeString(commentary);
        dest.writeString(scorecard);
        dest.writeString(schedule);
        dest.writeString(recent_results);
        dest.writeString(setting);
        dest.writeString(language);
        dest.writeString(feedback);
        dest.writeString(auto_refresh);
        dest.writeString(target);
        dest.writeString(ball_by_ball);
        dest.writeString(over_by_over);
        dest.writeString(squads);
        dest.writeString(match_info);
        dest.writeString(batsman);
        dest.writeString(bowler);
        dest.writeString(partnership);
        dest.writeString(last_wkt);
        dest.writeString(runs);
        dest.writeString(recent_over);
        dest.writeString(series_name);
        dest.writeString(toss);
        dest.writeString(venue);
        dest.writeString(date);
        dest.writeString(teams);
        dest.writeString(umpires);
        dest.writeString(umpire_1);
        dest.writeString(umpire_2);
        dest.writeString(umpire_3);
        dest.writeString(referee);
        dest.writeString(mom);
        dest.writeString(mos);
        dest.writeString(extras);
        dest.writeString(extras_b);
        dest.writeString(extras_wd);
        dest.writeString(extras_nb);
        dest.writeString(extras_lb);
        dest.writeString(extras_p);
        dest.writeString(yet_to_bat);
        dest.writeString(header_crr);
        dest.writeString(header_rr);
        dest.writeString(header_run);
        dest.writeString(header_ball);
        dest.writeString(header_4s);
        dest.writeString(header_6s);
        dest.writeString(header_sr);
        dest.writeString(header_over);
        dest.writeString(header_overs);
        dest.writeString(header_maiden);
        dest.writeString(header_wickets);
        dest.writeString(header_eco);
        dest.writeString(fall_of_wicket);
        dest.writeString(name);
        dest.writeString(email);
        dest.writeString(subject);
        dest.writeString(message);
        dest.writeString(powerplay);
        dest.writeString(mandatory);
        dest.writeString(batting);
        dest.writeString(bowling);
        dest.writeString(month_jan);
        dest.writeString(month_feb);
        dest.writeString(month_mar);
        dest.writeString(month_apr);
        dest.writeString(month_may);
        dest.writeString(month_jun);
        dest.writeString(month_jul);
        dest.writeString(month_aug);
        dest.writeString(month_sep);
        dest.writeString(month_oct);
        dest.writeString(month_nov);
        dest.writeString(month_dec);
        dest.writeString(week_sun);
        dest.writeString(week_mon);
        dest.writeString(week_tue);
        dest.writeString(week_wed);
        dest.writeString(week_thu);
        dest.writeString(week_fri);
        dest.writeString(week_sat);
        dest.writeString(pull_to_refresh);
        dest.writeString(loading);
        dest.writeString(dob);
        dest.writeString(birth_place);
        dest.writeString(batting_style);
        dest.writeString(bowling_style);
        dest.writeString(ranking);
        dest.writeString(career);
        dest.writeString(rate_us);
        dest.writeString(yes);
        dest.writeString(no);
        dest.writeString(no_internet);
        dest.writeString(thank_you_contacting);
        dest.writeString(career_best);
        dest.writeString(enter_valid_email);
        dest.writeString(fill_all_details);
        dest.writeString(release_to_refresh);
        dest.writeString(bench);
        dest.writeString(career_stats);
        dest.writeString(current_series);
        dest.writeString(info);
        dest.writeString(test);
        dest.writeString(odi);
        dest.writeString(t20);
        dest.writeString(ipl);
        dest.writeString(cl);
        dest.writeString(debut);
        dest.writeString(last_played);
        dest.writeString(format);
        dest.writeString(innings_list);
        dest.writeString(overs);
        dest.writeString(venue_time);
        dest.writeString(teams_played_for);
        dest.writeString(starts_in);
        dest.writeString(follow_on);
        dest.writeString(decl);
        dest.writeString(points_table);
        dest.writeString(country);
        dest.writeString(points);
        dest.writeString(rank);
        dest.writeString(rating);
        dest.writeString(matches);
        dest.writeString(header_avg);
        dest.writeString(top_teams);
        dest.writeString(top_batsman);
        dest.writeString(top_bowlers);
        dest.writeString(top_allrounder);
        dest.writeString(player);
    }

    @Override
    public String toString() {
        return "DictonaryPojo{" +
                "home='" + home + '\'' +
                ", commentary='" + commentary + '\'' +
                ", scorecard='" + scorecard + '\'' +
                ", schedule='" + schedule + '\'' +
                ", recent_results='" + recent_results + '\'' +
                ", setting='" + setting + '\'' +
                ", language='" + language + '\'' +
                ", feedback='" + feedback + '\'' +
                ", auto_refresh='" + auto_refresh + '\'' +
                ", target='" + target + '\'' +
                ", ball_by_ball='" + ball_by_ball + '\'' +
                ", over_by_over='" + over_by_over + '\'' +
                ", squads='" + squads + '\'' +
                ", match_info='" + match_info + '\'' +
                ", batsman='" + batsman + '\'' +
                ", bowler='" + bowler + '\'' +
                ", partnership='" + partnership + '\'' +
                ", last_wkt='" + last_wkt + '\'' +
                ", runs='" + runs + '\'' +
                ", recent_over='" + recent_over + '\'' +
                ", series_name='" + series_name + '\'' +
                ", toss='" + toss + '\'' +
                ", venue='" + venue + '\'' +
                ", date='" + date + '\'' +
                ", teams='" + teams + '\'' +
                ", umpires='" + umpires + '\'' +
                ", umpire_1='" + umpire_1 + '\'' +
                ", umpire_2='" + umpire_2 + '\'' +
                ", umpire_3='" + umpire_3 + '\'' +
                ", referee='" + referee + '\'' +
                ", mom='" + mom + '\'' +
                ", mos='" + mos + '\'' +
                ", extras='" + extras + '\'' +
                ", extras_b='" + extras_b + '\'' +
                ", extras_wd='" + extras_wd + '\'' +
                ", extras_nb='" + extras_nb + '\'' +
                ", extras_lb='" + extras_lb + '\'' +
                ", extras_p='" + extras_p + '\'' +
                ", yet_to_bat='" + yet_to_bat + '\'' +
                ", header_crr='" + header_crr + '\'' +
                ", header_rr='" + header_rr + '\'' +
                ", header_run='" + header_run + '\'' +
                ", header_ball='" + header_ball + '\'' +
                ", header_4s='" + header_4s + '\'' +
                ", header_6s='" + header_6s + '\'' +
                ", header_sr='" + header_sr + '\'' +
                ", header_over='" + header_over + '\'' +
                ", header_overs='" + header_overs + '\'' +
                ", header_maiden='" + header_maiden + '\'' +
                ", header_wickets='" + header_wickets + '\'' +
                ", header_eco='" + header_eco + '\'' +
                ", fall_of_wicket='" + fall_of_wicket + '\'' +
                ", name='" + name + '\'' +
                ", email='" + email + '\'' +
                ", subject='" + subject + '\'' +
                ", message='" + message + '\'' +
                ", powerplay='" + powerplay + '\'' +
                ", mandatory='" + mandatory + '\'' +
                ", batting='" + batting + '\'' +
                ", bowling='" + bowling + '\'' +
                ", month_jan='" + month_jan + '\'' +
                ", month_feb='" + month_feb + '\'' +
                ", month_mar='" + month_mar + '\'' +
                ", month_apr='" + month_apr + '\'' +
                ", month_may='" + month_may + '\'' +
                ", month_jun='" + month_jun + '\'' +
                ", month_jul='" + month_jul + '\'' +
                ", month_aug='" + month_aug + '\'' +
                ", month_sep='" + month_sep + '\'' +
                ", month_oct='" + month_oct + '\'' +
                ", month_nov='" + month_nov + '\'' +
                ", month_dec='" + month_dec + '\'' +
                ", week_sun='" + week_sun + '\'' +
                ", week_mon='" + week_mon + '\'' +
                ", week_tue='" + week_tue + '\'' +
                ", week_wed='" + week_wed + '\'' +
                ", week_thu='" + week_thu + '\'' +
                ", week_fri='" + week_fri + '\'' +
                ", week_sat='" + week_sat + '\'' +
                ", pull_to_refresh='" + pull_to_refresh + '\'' +
                ", loading='" + loading + '\'' +
                ", dob='" + dob + '\'' +
                ", birth_place='" + birth_place + '\'' +
                ", batting_style='" + batting_style + '\'' +
                ", bowling_style='" + bowling_style + '\'' +
                ", ranking='" + ranking + '\'' +
                ", career='" + career + '\'' +
                ", rate_us='" + rate_us + '\'' +
                ", yes='" + yes + '\'' +
                ", no='" + no + '\'' +
                ", no_internet='" + no_internet + '\'' +
                ", thank_you_contacting='" + thank_you_contacting + '\'' +
                ", career_best='" + career_best + '\'' +
                ", enter_valid_email='" + enter_valid_email + '\'' +
                ", fill_all_details='" + fill_all_details + '\'' +
                ", release_to_refresh='" + release_to_refresh + '\'' +
                ", bench='" + bench + '\'' +
                ", career_stats='" + career_stats + '\'' +
                ", current_series='" + current_series + '\'' +
                ", info='" + info + '\'' +
                ", test='" + test + '\'' +
                ", odi='" + odi + '\'' +
                ", t20='" + t20 + '\'' +
                ", ipl='" + ipl + '\'' +
                ", cl='" + cl + '\'' +
                ", debut='" + debut + '\'' +
                ", last_played='" + last_played + '\'' +
                ", format='" + format + '\'' +
                ", innings_list='" + innings_list + '\'' +
                ", overs='" + overs + '\'' +
                ", venue_time='" + venue_time + '\'' +
                ", teams_played_for='" + teams_played_for + '\'' +
                ", starts_in='" + starts_in + '\'' +
                ", follow_on='" + follow_on + '\'' +
                ", decl='" + decl + '\'' +
                ", points_table='" + points_table + '\'' +
                ", country='" + country + '\'' +
                ", points='" + points + '\'' +
                ", rank='" + rank + '\'' +
                ", rating='" + rating + '\'' +
                ", matches='" + matches + '\'' +
                ", header_avg='" + header_avg + '\'' +
                ", top_teams='" + top_teams + '\'' +
                ", top_batsman='" + top_batsman + '\'' +
                ", top_bowlers='" + top_bowlers + '\'' +
                ", top_allrounder='" + top_allrounder + '\'' +
                ", player='" + player + '\'' +
                '}';
    }
}
